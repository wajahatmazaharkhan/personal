...
import { engagementItems } from "../data/engagement";
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 animate-fade-in">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {engagementItems.map((item, i) => (
          <div
            key={i}
            className="group overflow-hidden rounded-2xl border border-[var(--border-color)] bg-[var(--card-color)] hover:shadow-xl transition-all"
          >
            ...
            <div className="p-6">
              <span className="text-sm font-semibold text-[var(--text-muted-color)]">
                {item.date}
              </span>
              <h3 className="text-xl font-bold mt-2 group-hover:text-[var(--color-primary)] transition-colors">
                {item.title}
              </h3>
